...
MAX_MB = 20  # limite de upload (MB)
ALLOWED_ORIGINS = {"https://natanjs01.github.io"}

# flask-cors (principal)
CORS(
    app,
...
# garantia extra de CORS em todas as respostas (inclui erros)
@app.after_request
...
def pdf_to_docx(file_stream) -> bytes:
    """Converte PDF em DOCX simples (texto por linha com cabeçalho de página)."""
    doc = Document()
...
def _sanitize_cell(s: str) -> str:
    """Remove caracteres inválidos para XML do XLSX e normaliza espaços."""
    if s is None:
        return ""
...
def _build_grid_by_gaps(words, max_cols=18):
    ...
    if len(bounds) - 1 > max_cols:
        ...
        for _ in range(max_cols - 1):
            acc += step
            nb.append(bounds[int(round(acc))])
        nb.append(bounds[-1])
        bounds = nb
...
def _materialize_rpt_lojas(words) -> List[List[str]]:
    ...
    for line in lines:
        ...
        up = txt.upper()
        if up.startswith("LOJA "):
            loja_atual = _extract_loja_name(line)
            continue
        if "TOTAL" in up or up.startswith("PÁGINA") or up.startswith("RELAT") or up.startswith("DATA:"):
            continue
